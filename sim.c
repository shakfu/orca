...
static ORCA_PURE bool oper_has_neighboring_bang(Glyph const* gbuf, Usz h,
                                                Usz w, Usz y, Usz x)
{
    ...
    // the arithmetic under/overflows, even if guarded the guard on lhs is
    // false
    if (y > 0 && *(gp - w) == '*')
        return true;
    return false;
...
static U8 midi_note_number_of(Glyph g)
{
    ...
    return (U8)(deg / 7 * 12 + (I8[]) { 0, 2, 4, 5, 7, 9, 11 }[deg % 7]
                + sharp);
}
...
static void oper_poke_and_stun(Glyph* restrict gbuffer, Mark* restrict mbuffer,
                               Usz height, Usz width, Usz y, Usz x,
                               Isz delta_y, Isz delta_x, Glyph g)
{
...
if (channel > 15)
    return;
PORT(0, 0, OUT);
Oevent_midi_cc* oe = (Oevent_midi_cc*)oevent_list_alloc_item(
    extra_params->oevent_list);
...
if (velocity_g == '.') {
    ...
    // to send note offs. Zero-velocity is the alternate way.) If there is a
    // zero velocity, we'll just not do anything.
    if (vel_num == 0)
        return;
...
PORT(0, 0, OUT);
Oevent_midi_note* oe = (Oevent_midi_note*)oevent_list_alloc_item(
    extra_params->oevent_list);
...
STOP_IF_NOT_BANGED;
PORT(0, 0, OUT);
Oevent_udp_string* oe = (Oevent_udp_string*)oevent_list_alloc_item(
    extra_params->oevent_list);
...
if (g != '.') {
    ...
    Oevent_osc_ints* oe = &oevent_list_alloc_item(extra_params->oevent_list)
                               ->osc_ints;
...
if (channel > 15)
    return;
PORT(0, 0, OUT);
Oevent_midi_pb* oe = (Oevent_midi_pb*)oevent_list_alloc_item(
    extra_params->oevent_list);
...
#define UNIQUE_CASE(_oper_char, _oper_name)                                   \
    case _oper_char:                                                          \
        oper_behavior_##_oper_name(gbuf, mbuf, height, width, iy, ix,         \
                                   tick_number, &extras, cell_flags,          \
                                   glyph_char);                               \
        break;
