        ...
        for (int ifg = 0; ifg < Colors_count; ++ifg) {
            for (int ibg = 0; ibg < Colors_count; ++ibg) {
                int res = init_pair((short int)(1 + ifg * Colors_count + ibg),
                                    ...
                                    (short int)(ibg - 1));
                (void)res;
                // Might fail on Linux virtual console/terminal for a couple of
                // colors. Just ignore.
#if 0
        if (res == ERR) {
          endwin();
...
static ORCA_NOINLINE void qnav_reposition_block(Qblock* qb)
{
    ...
    if (left + total_w > term_w) {
        // If we have enough room if we position just below the previous item
        // in the stack, do that instead of positioning to the right of it.
        if (prev_x + total_w <= term_w
            && total_h < term_h - (prev_y + prev_h)) {
            top = prev_y + prev_h;
            left = prev_x;
        }
...
Qmsg* qmsg_printf_push(char const* title, char const* fmt, ...)
{
    ...
    Qmsg* msg = qmsg_push(lines,
                          width); // no wrapping yet, no real wcwidth, etc
...
{
    // Probably a programming error if there are no items. Make the menu
    // visible so the programmer knows something went wrong.
    if (qm->items_count == 0)
        qmenu_add_spacer(qm);
    ...
    if (qm->qblock.title) {
        // Stupid lack of wcswidth() means we can't know how wide this string
        // is actually displayed. Just fake it for now, until we have Unicode
        // strings in the UI. Then we get sad.
