                               ...
                               bool is_playing)
{
    ...
    // easier than pulling the glyphs from the gbuffer, since we already did
    // the ruler calculations to turn . into +, and we don't need special
    // behavior for any other attributes (e.g. we don't show a special state
    // for selected uppercase characters.)
    //
    ...
    // tricky since we have to handle it being partially visible from any edge
    // on any axis, and we have to be mindful overflow.
    Usz vis_sel_y;
    Usz vis_sel_x;
    ...
    for (Usz iy = 0; iy < vis_sel_h; ++iy) {
        ...
        int num = mvwinchnstr(win, at_y, (int)vis_sel_x, chbuffer,
                              (int)vis_sel_w);
        for (int ix = 0; ix < num; ++ix) {
            chbuffer[ix] = (chtype)((chbuffer[ix]
...
staticni void print_activity_indicator(WINDOW* win, Usz activity_counter)
{
    ...
    // If you want to see what various combinations of colors and attributes
    // look like in different terminals.
#if 0
...
staticni void draw_hud(WINDOW* win, int win_y, int win_x, int height,
                       int width, char const* filename, Usz field_h,
                       Usz field_w, Usz ruler_spacing_y, Usz ruler_spacing_x,
                       Usz tick_num, Usz bpm, Ged_cursor const* ged_cursor,
                               ...
                               bool use_fancy_dots, bool use_fancy_rulers)
{
    ...
    for (Usz iy = 0; iy < rows; ++iy) {
        ...
        bool use_y_ruler = use_rulers
            && (iy + offset_y) % ruler_spacing_y == 0;
...
staticni void draw_oevent_list(WINDOW* win, Oevent_list const* oevent_list)
{
    ...
    for (Usz i = 0, num_events = oevent_list->count; i < num_events; ++i) {
        ...
        case Oevent_type_midi_note: {
            ...
            wprintw(win,
                    "MIDI Note\tchannel %d\toctave %d\tnote %d\tvelocity "
                    "%d\tlength %d",
        ...
        case Oevent_type_midi_pb: {
            ...
            wprintw(win, "MIDI PB\tchannel %d\tmsb %d\tlsb %d",
                    (int)ep->channel, (int)ep->msb, (int)ep->lsb);
            break;
        }
...
staticni void ged_resize_grid(Field* field, Mbuf_reusable* mbr, Usz new_height,
                              Usz new_width, Usz tick_num,
                              Field* scratch_field, Undo_history* undo_hist,
                                         ...
                                         Ged_cursor* ged_cursor)
{
    ...
    if (new_field_h == field_h && new_field_w == field_w)
        return false;
    ged_resize_grid(field, mbr, new_field_h, new_field_w, tick_num,
...
staticni PmError midi_mode_init_portmidi(Midi_mode* mm, PmDeviceID dev_id)
{
    ...
    if (e)
        goto fail;
    e = Pm_OpenOutput(&mm->portmidi.stream, dev_id, NULL, 128,
                      portmidi_timeproc, NULL, Portmidi_artificial_latency);
    if (e)
        goto fail;
...
static bool portmidi_find_name_of_device_id(PmDeviceID id,
                                            PmError* out_pmerror,
                                            oso** out_name)
{
    *out_pmerror = portmidi_init_if_necessary();
...
staticni void midi_mode_deinit(Midi_mode* mm)
{
    ...
    case Midi_mode_type_portmidi:
        // Because PortMidi seems to work correctly ony more platforms when
        // using its timing stuff, we are using it. And because we are using
        // it, and because it may be buffering events for sending 'later', we
        // might have pending outgoing MIDI events. We'll need to wait until
        // they finish being before calling Pm_Close, otherwise users could
        // have problems like MIDI notes being stuck on. This is slow and
        // blocking, but not much we can do about it right now.
        //
...
#ifdef FEAT_PORTMIDI
    case Midi_mode_type_portmidi: {
        // timestamp is totally fake, to prevent problems with some MIDI
        // systems getting angry if there's no timestamping info.
        //
        // Eventually, we will want to create real timestamps based on a real
        // orca clock, instead of ad-hoc at the last moment like this. When we
        // do that, we'll need to thread the timestamping/timing info through
        // the function calls, instead of creating it at the last moment here.
        // (This timestamp is actually 'useless', because it doesn't convey any
        // additional information. But if we don't provide it, at least to
        // PortMidi, some people's MIDI setups may malfunction and have
        // terrible timing problems.)
    ...
    for (Usz i = 0; i < count; ++i) {
        ...
        case Oevent_type_midi_note: {
            ...
            if (channel > 15)
                break;
            if (em->mono) {
                // 'mono' note-ons are strange. The more typical branch you'd
                // expect to see, where you can play multiple notes per
                // channel, is below.
                monofied_chans |= 1u << (channel & 0xFu);
                midi_mono_ons[channel] = (Midi_mono_on) {
                    .note_number = (U8)note_number,
                    .velocity = em->velocity,
                    .duration = em->duration
                };
            } else {
                midi_note_ons[midi_note_count] = (Midi_note_on) {
                    .channel = (U8)channel,
                    .note_number = (U8)note_number,
                    .velocity = em->velocity
                };
        ...
        case Oevent_type_midi_cc: {
            ...
            // emitted by the orca VM. Notes and CCs that are emitted in the
            // same step will always have the CCs sent first. Not sure if this
            // is OK or not. If it's not OK, we can either loop again a second
            // time to always send CCs after notes, or if that's not also OK,
            // we can make the stack buffer more complicated and interleave the
            // CCs in it.
            send_midi_chan_msg(oosc_dev, midi_mode, 0xb, ec->channel,
                               ec->control, ec->value);
            break;
        }
        case Oevent_type_midi_pb: {
...
do_note_ons:
    if (midi_note_count > 0) {
        ...
        if (start_note_offs != end_note_offs) {
            Susnote const* restrict susnotes_off = susnote_list->buffer;
            send_midi_note_offs(oosc_dev, midi_mode,
    ...
    if (monofied_chans) {
        // The behavior we end up with is that if regular note-ons are played
        // in the same frame/step as a mono, the regular note-ons will have the
        // actual MIDI note on sent, followed immediately by a MIDI note off. I
        ...
        for (Usz i = 0; i < 16; i++) { // Add these notes to list of note-ons
            ...
            new_susnotes[midi_note_count] = (Susnote) {
                .remaining = (float)(frame_secs
                                     * (double)midi_mono_ons[i].duration),
                            ...
                            char const* dest_port)
{
    ged_clear_osc_udp(a);
    if (dest_port) {
        Oosc_udp_create_error err = oosc_dev_create_udp(&a->oosc_dev,
                                                        dest_addr, dest_port);
        if (err) {
            return false;
        }
...
static double ged_secs_to_deadline(Ged const* a)
{
    ...
    // for ORCA's timing semantics, divide it by a further 6. This same logic
    // is mirrored in ged_do_stuff().
    if (a->midi_bclock)
        secs_span /= 6.0;
                       ...
                       bool use_fancy_dots, bool use_fancy_rulers)
{
    ...
    // phase 0 of the simulation run, which means the ports and other flags
    // won't be set on the mark buffer, so the colors for disabled cells,
    // ports, etc. won't be set.
    //
    // We can just perform a simulation step using the current state, keep the
    // mark buffer that it produces, then roll back the glyph buffer to where
    // it was before. This should produce results similar to having specialized
    // UI code that looks at each glyph and figures out the ports, etc.
    if (a->needs_remarking && !a->is_playing) {
        ...
        clear_and_run_vm(a->scratch_field.buffer, a->mbuf_r.buffer,
                         a->field.height, a->field.width, a->tick_num,
                         &a->scratch_oevent_list, a->random_seed);
    ...
    draw_glyphs_grid_scrolled(
        ...
        a->ruler_spacing_y, a->ruler_spacing_x, use_fancy_dots,
        use_fancy_rulers);
...
staticni bool ged_slide_selection(Ged* a, int delta_y, int delta_x)
{
    ...
    gbuffer_copy_subrect(a->field.buffer, a->field.buffer, field_h, field_w,
                         field_h, field_w, curs_y_0, curs_x_0, curs_y_1,
                         curs_x_1, curs_h_0, curs_w_0);
    // Erase/clear the area that was within the selection rectangle in the
    // starting position, but wasn't written to during the copy. (In other
    // words, this is the area that was 'left behind' when we moved the
    // selection rectangle, plus any area that was along the bottom and right
    // edge of the field that didn't have anything to copy to it when the
    // selection rectangle extended outside of the field.)
    Usz ey, eh, ex, ew;
                                                ...
                                                mmask_t mouse_bstate)
{
    if (mouse_bstate & BUTTON1_RELEASED) {
        ...
        // mousemask() with our original parameters seems to work to get into
        // the state we want, though.
        //
        // printf("\033[?1003l\n");
        ...
        Usz y = view_to_scrolled_grid(a->field.height, vis_y,
                                      a->grid_scroll_y);
        Usz x = view_to_scrolled_grid(a->field.width, vis_x, a->grid_scroll_x);
        if (!a->is_mouse_down) {
            // some sequence to hopefully make terminal start reporting all
            // further mouse movement events. 'REPORT_MOUSE_POSITION' alone in
            // the mousemask doesn't seem to work, at least not for xterm. we
            // need to set it only only when needed, otherwise some terminals
            // will send movement updates when we don't want them.
            printf("\033[?1003h\n");
            // need to do this or double clicking can cause terminal state to
            // get corrupted, since we're bypassing curses here. might cause
            // flicker. wish i could figure out why report mouse position isn't
            // working on its own.
            fflush(stdout);
            wclear(stdscr);
...
staticni void ged_resize_grid_relative(Ged* a, Isz delta_y, Isz delta_x)
{
    ged_resize_grid_snap_ruler(&a->field, &a->mbuf_r, a->ruler_spacing_y,
                               a->ruler_spacing_x, delta_y, delta_x,
                               a->tick_num, &a->scratch_field, &a->undo_hist,
                               &a->ged_cursor);
...
staticni void ged_write_character(Ged* a, char c)
{
    ...
    if (a->input_mode == Ged_input_mode_append) {
        ged_cursor_move_relative(&a->ged_cursor, a->field.height,
                                 a->field.width, 0, 1);
    }
    a->is_draw_dirty = true;
}
...
staticni bool ged_fill_selection_with_char(Ged* a, Glyph c)
{
    ...
    if (!ged_try_selection_clipped_to_field(a, &curs_y, &curs_x, &curs_h,
                                            &curs_w))
        return false;
    gbuffer_fill_subrect(a->field.buffer, a->field.height, a->field.width,
                         curs_y, curs_x, curs_h, curs_w, c);
    return true;
}
...
static void push_osc_menu(bool output_enabled)
{
    ...
    qmenu_add_printf(qm, Osc_menu_output_enabledisable,
                     "[%c] OSC Output Enabled", output_enabled ? '*' : ' ');
...
{
    qform_single_line_input(Osc_output_address_form_id,
                            "Set OSC Output Address", initial);
}
...
static void push_playback_menu(bool midi_bclock_enabled)
{
    ...
    qmenu_add_printf(qm, Playback_menu_midi_bclock,
...
static void push_controls_msg(void)
{
    ...
    for (Usz i = 0; i < ORCA_ARRAY_COUNTOF(items); ++i) {
        // use wcswidth instead of strlen if you need wide char support. but
        // note that won't be useful for UTF-8 or unicode chars in higher plane
        // (emoji, complex zwj, etc.)
        if (items[i].input) {
...
{
    char buff[128];
    int snres = snprintf(buff, sizeof buff, "%zux%zu", init_width,
                         init_height);
                                        ...
                                        bool* io_use_gui_clipboard)
{
  ...
  Cboard_error cberr =
    ...
    if (cb_h < 1 || cb_w < 1)
        return;
    Cboard_error cberr = cboard_copy(a->clipboard_field.buffer, cb_h, cb_w, 0,
                                     0, cb_h, cb_w);
    if (cberr)
...
staticni void tui_save_prefs(Tui* t)
{
    ...
    case Midi_mode_type_portmidi: {
        ...
        if (!portmidi_find_name_of_device_id(
                t->ged.midi_mode.portmidi.device_id, &pmerror,
    ...
    // "Touched" items include conf items that were present on disk when we
    // first loaded the config file, plus the items that the user has modified
    // by interacting with the menus.
    ...
    while (ezconf_w_step(&ez)) {
        ...
        case Confopt_grid_dot_type:
            ...
            break;
        case Confopt_grid_ruler_type:
            fputs(t->fancy_grid_rulers ? prefval_fancy : prefval_plain,
                  ez.file);
            break;
        }
    }
...
staticni bool tui_restart_osc_udp_if_enabled_diderror(Tui* t)
{
    ...
    if (t->osc_output_enabled && t->osc_port) {
        error = !ged_set_osc_udp(&t->ged,
                                 osoc(t->osc_address) /* null ok here */,
                                 osoc(t->osc_port));
    } else {
...
{
    qmsg_printf_push("OSC Networking Error",
                     "Failed to set up OSC networking");
}
...
staticni void tui_adjust_term_size(Tui* t, WINDOW** cont_window)
{
    ...
    if (remake_window) {
        if (*cont_window)
            delwin(*cont_window);
        wclear(stdscr);
        *cont_window = derwin(stdscr, content_h, content_w, content_y,
                              content_x);
        t->ged.is_draw_dirty = true;
    }
        ...
        case Qmenu_action_type_picked:
            ...
            case Main_menu_id:
                ...
                case Main_menu_set_grid_dims:
                    push_set_grid_dims_form(t->ged.field.height,
                                            t->ged.field.width);
                    break;
            ...
            case Autofit_menu_id: {
                ...
                case Autofit_nicely_id:
                    did_get_ok_size = tui_suggest_nice_grid_size(
                        t, t->ged.win_h, t->ged.win_w, &new_field_h,
                        &new_field_w);
                    break;
            ...
            case Confirm_new_file_menu_id:
                ...
                case Confirm_new_file_accept_id: {
                    Usz new_field_h, new_field_w;
                    if (tui_suggest_nice_grid_size(t, t->ged.win_h,
                                                   t->ged.win_w, &new_field_h,
                                                   &new_field_w)) {
                        undo_history_push(&t->ged.undo_hist, &t->ged.field,
                                          t->ged.tick_num);
                        field_resize_raw(&t->ged.field, new_field_h,
                                         new_field_w);
            ...
            case Cosmetics_menu_id:
                ...
                case Cosmetics_grid_dots_id:
                    push_plainorfancy_menu(Set_fancy_grid_dots_menu_id,
                                           "Grid Dots", t->fancy_grid_dots);
                    break;
            ...
            case Playback_menu_id:
                ...
                case Playback_menu_midi_bclock: {
                    ...
                    if (t->ged.is_playing) {
                        int msgbyte = new_enabled ? 0xFA /* start */
                                                  : 0xFC /* stop */;
                        send_midi_byte(t->ged.oosc_dev, &t->ged.midi_mode,
                                       msgbyte);
                        // TODO timing judder will be experienced here, because
                        // the deadline calculation conditions will have been
                        // changed by toggling the midi_bclock flag. We would
                        // have to transfer the current remaining time from the
                        // reference clock point into the accum time, and
                        // mutiply or divide it.
                    }
            ...
            case Set_fancy_grid_rulers_menu_id:
                plainorfancy_menu_was_picked(
                    t, act.picked.id, &t->fancy_grid_rulers,
            ...
            case Osc_menu_id:
                ...
                case Osc_menu_output_address:
                    push_osc_output_address_form(
                        osoc(t->osc_address) /* null ok */);
                    break;
            ...
            case Portmidi_output_device_menu_id: {
                ...
                midi_mode_deinit(&t->ged.midi_mode);
                PmError pme = midi_mode_init_portmidi(&t->ged.midi_mode,
                                                      act.picked.id);
                qnav_stack_pop();
                if (pme) {
                    qmsg_printf_push(
            ...
            case Qform_action_type_submitted: {
                ...
                case Open_form_id: {
                    ...
                    bool added_hist = undo_history_push(
                        &t->ged.undo_hist, &t->ged.field, t->ged.tick_num);
                    Field_load_error fle = field_load_file(osoc(temp_name),
                                                           &t->ged.field);
                    if (fle == Field_load_error_ok) {
                        qnav_stack_pop();
                        osoputoso(&t->file_name, temp_name);
                        mbuf_reusable_ensure_size(&t->ged.mbuf_r,
                                                  t->ged.field.height,
                                                  t->ged.field.width);
                        ged_cursor_confine(&t->ged.ged_cursor,
                                           t->ged.field.height,
                                           t->ged.field.width);
                        ged_update_internal_geometry(&t->ged);
                        ged_make_cursor_visible(&t->ged);
                ...
                case Save_as_form_id: {
                    ...
                    if (!temp_name)
                        break;
                    qnav_stack_pop();
                    bool saved_ok = try_save_with_msg(&t->ged.field,
                                                      temp_name);
                    if (saved_ok)
                        osoputoso(&t->file_name, temp_name);
                    osofree(temp_name);
                ...
                case Set_grid_dims_form_id: {
                    ...
                    if (sscanf(osoc(tmpstr), "%dx%d", &newwidth, &newheight)
                            == 2
                        && newheight > 0 && newwidth > 0
                        && newheight < ORCA_Y_MAX && newwidth < ORCA_X_MAX) {
                        if (t->ged.field.height != (Usz)newheight
                            || t->ged.field.width != (Usz)newwidth) {
                            ged_resize_grid(
                                &t->ged.field, &t->ged.mbuf_r, (Usz)newheight,
...
int main(int argc, char** argv)
{
    static struct option tui_options[] = {
        ...
        { "portmidi-list-devices", no_argument, 0,
          Argopt_portmidi_deprecated },
    ...
    {                                                                         \
        ...
        case Argopt_osc_deprecated:
            fprintf(stderr,
                    ...
                    "Instead, set the OSC server and port from within the "
                    "ORCA menu.\n");
            exit(1);
        }
    }
#undef OPTFAIL
    ...
    if (osolen(t.osc_midi_bidule_path) > 0) {
        midi_mode_deinit(&t.ged.midi_mode);
        midi_mode_init_osc_bidule(&t.ged.midi_mode,
                                  osoc(t.osc_midi_bidule_path));
    }
    ...
    // Allow ncurses to control newline translation. Fine to use with any
    // modern terminal, and will let ncurses run faster.
    nonl();
    ...
    // ctrl+z, ctrl+c etc. as input instead of having a signal generated. We
    // need to do this even with wtimeout() if we don't want ctrl+z etc. to
    ...
    if (osolen(t.file_name)) {
        Field_load_error fle = field_load_file(osoc(t.file_name),
                                               &t.ged.field);
        switch (fle) {
        case Field_load_error_ok:
            if (t.ged.field.height < 1 || t.ged.field.width < 1) {
                // Opening an empty file or attempting to open a directory can
                // lead us here.
        ...
        case Field_load_error_cant_open_file: {
            ...
            // differentiation between "file exists and can't open it" and
            // "file doesn't seem to exist."
            Qmsg* qm = qmsg_printf_push(NULL, "New file: %s",
                                        osoc(t.file_name));
            qmsg_set_dismiss_mode(qm, Qmsg_dismiss_mode_passthrough);
            break;
        }
    ...
    if (!grid_initialized) {
        ...
        if (explicit_initial_grid_size
            || !tui_suggest_nice_grid_size(&t, t.ged.win_h, t.ged.win_w,
...
#define DEADTIME(_millisecs, _new_timeout)                                    \
        ...
        int new_timeout;
        // These values are tuned to work OK with the normal scheduling
        // behavior on Linux, Mac, and Windows. All of the usual caveats of
        // trying to guess what the scheduler will do apply.
        //
        // Of course, there's no guarantee about how the scheduler will work,
        // so if you are using a modified kernel or something, or the
        // measurements here are bad, or it's some OS that behaves differently
        // than expected, this won't be very good. But there's not really much
        // we can do about it, and it's better than doing nothing and burning
        // up the CPU!
...
#undef DEADTIME
        ...
        if (key == 27 /* escape */) {
            if (brackpaste_seq_getungetch(stdscr) == Brackpaste_seq_end) {
                ...
                if (brackpaste_max_y > t.ged.ged_cursor.y)
                    t.ged.ged_cursor.h = brackpaste_max_y - t.ged.ged_cursor.y
                        + 1;
                if (brackpaste_max_x > t.ged.ged_cursor.x)
                    t.ged.ged_cursor.w = brackpaste_max_x - t.ged.ged_cursor.x
                        + 1;
                t.ged.needs_remarking = true;
                t.ged.is_draw_dirty = true;
            }
            ...
            if (key != ' ') {
                ...
                if (brackpaste_y < t.ged.field.height
                    ...
                    gbuffer_poke(t.ged.field.buffer, t.ged.field.height,
                                 t.ged.field.width, brackpaste_y, brackpaste_x,
                                 cleaned);
                    // Could move this out one level if we wanted the final
                    // selection size to reflect even the pasted area which
                    // didn't fit on the grid.
    ...
    case '/':
        ...
        // input a '?' instead of a '/' because '?' opens the help guide, and
        // it might be a bad idea to take that away, since orca will take over
        // the TTY and may leave users confused. I know of at least 1 person
        // who was saved by pressing '?' after they didn't know what to do.
        // Hmm.
        ged_input_character(&t.ged, '?');
        break;
    case '<':
