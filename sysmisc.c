...
{
    // a0 and a1 are the start and end positions of the left side of an
    // "foo=bar" pair. b0 and b1 are the positions right side. Leading and
                     ...
                     char* buf, Usz bufsize, Usz* out_index, char** out_value)
{
    ...
    Usz leftsz, rightsz;
next_line:;
    Conf_read_result res = conf_read_line(file, buf, bufsize, &left, &leftsz,
                                          out_value, &rightsz);
    switch (res) {
...
Conf_save_start_error conf_save_start(Conf_save* p, char const* conf_file_name)
{
    *p = (Conf_save) { 0 };
    ...
    if (!p->tempfile) {
        // Try to create config dir, in case it doesn't exist. (XDG says we
        // should do this, and use mode 0700.)
        mkdir(osoc(dir), 0700);
        p->tempfile = fopen(osoc(p->temppath), "w");
    }
    if (!p->tempfile) {
...
Conf_save_commit_error conf_save_commit(Conf_save* p)
{
    ...
    // simultaneously, I think we have to use OS-specific facilities. So I
    // guess this is the best we can do for now. I could be wrong, though. But
    // I couldn't find any good information about it.
...
bool ezconf_w_step(Ezconf_w* ezcw)
{
    ...
    if (ezcw->error || !tempfile) // Already errored or finished ok
        return false;
    // If we set a flag to write a closing newline the last time we were
    // called, write it now.
    ...
    for (;;) { // Scan through file looking for known keys in key=value lines
        ...
        Conf_read_result res = conf_read_line(origfile, linebuff,
                                              sizeof linebuff, &left, &leftsz,
                                              &right, &rightsz);
        switch (res) {
        case Conf_read_left_and_right: {
            for (size_t i = 0; i < optscount; i++) {
                ...
                if (strcmp(name, left) != 0)
                    continue;
                // If we already wrote this one, comment out the line instead,
...
write_leftovers: // Write out any guys that weren't in original file.
    for (;;) {   // Find the first guy that wasn't already written.
        if (!optscount)
            goto commit;
        chosen = opts;
        // Drop the guy from the front of the list. This is to reduce
        // super-linear complexity growth as the number of conf key-value pairs
        // are increased. (Otherwise, we iterate the full set of guys on each
        // call during the "write the leftovers" phase.)
        opts++;
        optscount--;
    ...
    // through as much of this list next time. (This might even end up
    // finishing it off, making it empty.)
    ezcw->opts = opts;
