                                          ...
                                          char const* dest_port)
{
    ...
    // Special behavior: if no hostname was provided, we'll get loopback(s)
    // from getaddrinfo. Which is good. But on systems with both an ipv4 and
    // ipv6 address, we might get the ipv6 address listed first. And the OSC
    // server, for example Plogue Bidule, might not support ipv6. And
    // defaulting to the ipv6 loopback wouldn't work, in that case. So if
    // there's no hostname, and we find an ipv4 address in the results, prefer
    // to use that.
    //
...
#endif
    int udpfd = socket(chosen->ai_family, chosen->ai_socktype,
                       chosen->ai_protocol);
    if (udpfd < 0) {
#if 0
